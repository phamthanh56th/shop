...
function folio_gallery_func($atts, $content = null){
    ...
    <div id="gallery" class="gallery full-gallery de-gallery pf_full_width <?php if ($columns == 2) {echo 'pf_2_cols'; }elseif ($columns == 3) { echo 'pf_3_cols'; }elseif ($columns == 5) { echo 'pf_5_cols'; }elseif ($columns == 6) { echo 'pf_6_cols'; }else{} ?> wow fadeInUp" data-wow-delay=".3s">
        <?php 
          	...
          	foreach((array)$cates as $cate){
              	if(count($cates)>0){	                
                	...
                	if( $link_video != '' ){
                		$js_cate_slug .= 'video-link-' . $cate->term_id .' ';                		
                	}else{
                		$js_cate_slug .= 'image-link-' . $cate->term_id .' ';
                	}                	   
              	} 
          	}          		          		  
        ?>             
        ...
        <div class="item <?php echo esc_attr($cate_slug); ?>">
            <div class="picframe">
                <a class="<?php echo $js_cate_slug; ?>" title="<?php the_title(); ?>" href="<?php if( $link_video != '' ){echo esc_url($link_video);}else{ echo esc_url(wp_get_attachment_url(get_post_thumbnail_id())); } ?>">
                    <span class="overlay">
                        <span class="pf_text">
	   		...
	   		<?php if($idcate != ''){ ?>
				<?php 
                    ...
                    foreach( (array)$idcate_categories as $idcate_categorie){
			            ...
			            $js_cat_slug = $js_categorie1->term_id; 
		        ?>							
        	...
        	<?php }else{ ?>
				<?php       
	                ...
	                foreach( (array)$js_categories as $js_categorie){		            
			            $js_cat_slug = $js_categorie->term_id;	      
	            ?>
	            	jQuery('.<?php echo 'image-link-' . esc_js($js_cat_slug); ?>, .<?php echo 'video-link-' . esc_js($js_cat_slug); ?>').magnificPopup({
					    callbacks:{
					      	elementParse: function(item) {
		            ...
		            <ul id="<?php echo esc_attr($filters_isotope); ?>" class="filters wow fadeInUp" data-wow-delay="0s">
		                ...
		                <?php       
		                    ...
		                    foreach( (array)$categories as $categorie){
					            ...
					            $cat_slug = $cates->term_id;	      
		                ?>
		                  <li><a href="#" data-filter=".category-<?php echo wp_specialchars_decode( $cat_slug ); ?>" <?php if($cate_active != '' && $cate_active == $cat_slug ){echo 'class="selected"';} ?> ><?php echo wp_specialchars_decode( $cat_name ); ?></a></li>
		                <?php } ?>                    
		            </ul>
		        </div>
		    </div>
				...
				$cate_slug_popup = '';
              		foreach((array)$cates as $cate){
              			if(count($cates)>0){
                			$cate_name .= $cate->name.' ' ;
                			$cate_slug .= 'category-'.$cate->term_id .' ';
							$cate_slug_popup .= 'ajax-popup-' . $cate->term_id . ' ';     
              			} 
          		}	
			}		
		?>          
